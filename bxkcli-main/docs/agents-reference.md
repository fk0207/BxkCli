...
1. `~/.bxkcli/config.json` 中对应 provider 的 `apiKey`
...
| 数据 | 默认路径 | 覆盖方式 |
|------|----------|----------|
| 长期记忆 | `~/.bxkcli/memory/long_term_memory.json` | `-Dbxkcli.memory.dir` |
| 项目级记忆 | `PAI.md` / `.bxkcli/PAI.md` / `PAI.local.md` | 用户级稳定偏好：`~/.bxkcli/PAI.md` |
| RAG 索引 | `~/.bxkcli/rag/codebase.db` | `-Dbxkcli.rag.dir` |
| 审计日志 | `~/.bxkcli/audit/audit-YYYY-MM-DD.jsonl` | `BXKCLI_AUDIT_DIR` / `-Dbxkcli.audit.dir` |
| Side-Git 快照 | `~/.bxkcli/snapshots/<project_hash>/<worktree_hash>/.git` | `BXKCLI_SNAPSHOT_DIR` / `-Dbxkcli.snapshot.dir` |
| 后台任务 | `~/.bxkcli/tasks/tasks.db` | — |

### Snapshot Config

系统属性 > 环境变量 > 默认值：`bxkcli.snapshot.enabled`(true) / `bxkcli.snapshot.max`(50) / `bxkcli.snapshot.excludes`(.git,.bxkcli/snapshots,target,node_modules,dist,.idea,*.class,*.jar) / `bxkcli.snapshot.dir`(~/.bxkcli/snapshots)
...
系统属性 > 环境变量/.env > 默认值：`BXKCLI_LOG_DIR`(~/.bxkcli/logs) / `BXKCLI_LOG_LEVEL`(INFO) / `BXKCLI_LOG_MAX_HISTORY`(7) / `BXKCLI_LOG_MAX_FILE_SIZE`(10MB) / `BXKCLI_LOG_TOTAL_SIZE_CAP`(100MB)

### ReAct/SubAgent Budget Config

系统属性 > 默认值：`bxkcli.react.token.budget`(Integer.MAX_VALUE) / `bxkcli.react.stagnation.window`(3) / `bxkcli.react.hard.max.iterations`(50)
...
系统属性 > 默认值：`bxkcli.llm.connect.timeout.seconds`(60) / `bxkcli.llm.read.timeout.seconds`(300) / `bxkcli.llm.write.timeout.seconds`(60) / `bxkcli.llm.call.timeout.seconds`(600)
...
1. 用户级：`~/.bxkcli/mcp.json`
2. 项目级：`.bxkcli/mcp.json`
...
- 加载顺序：`~/.bxkcli/PAI.md` → `PAI.md` → `.bxkcli/PAI.md` → `PAI.local.md` → `.bxkcli/PAI.local.md`
...
- shared 模式 close_page 只允许关闭 bxkcli 创建的 tab

### Skill System

- 三层加载：jar 内置 < 用户级 ~/.bxkcli/skills/ < 项目级 .bxkcli/skills/
...
- 环境变量：`BXKCLI_RENDERER=inline|lanterna|plain`
- `BXKCLI_TUI=true`(旧) → lanterna + deprecation 提示
- `BXKCLI_NO_STATUSBAR=true`：禁用底部状态栏
...
- `/export` 导出当前 ReAct conversationHistory 为 Markdown 到 `~/.bxkcli/exports/session-*.md`；包含完整 system prompt，便于检查 LLM 实际接收前的指令，命令不接受路径参数。
...
- `BXKCLI_LSP_ENABLED=false` 关闭

### Git Side-History Snapshot (Phase 18)

- side-git 在 ~/.bxkcli/snapshots/ 维护独立仓库（JGit，不依赖系统 git）
...
- 覆盖优先级：jar 内置 < 用户级 ~/.bxkcli/prompts/ < 项目级 .bxkcli/prompts/
...
- XfyunMaaSClient：Qwen3.6-35B-A3B，默认 https://maas-api.cn-huabei-1.xf-yun.com/v2，OpenAI-compatible 讯飞星辰 MaaS；可用 `/config provider xfyun ...` 写入配置后 `/model xfyun` 切换。`model` 必须使用 MaaS 服务管控页展示的 modelId；微调模型可配置 `--lora-id <resourceId>`，作为 HTTP header `lora_id` 发出；该 provider 不发送 bxkcli 内置 tools。
...
# BXKCLI_SNAPSHOT_ENABLED=true
# BXKCLI_SNAPSHOT_MAX=50
# BXKCLI_SNAPSHOT_EXCLUDES=.git,.bxkcli/snapshots,target,node_modules,dist,.idea,*.class,*.jar
# BXKCLI_SNAPSHOT_DIR=/Users/yourname/.bxkcli/snapshots
# BXKCLI_TUI=true
# NO_TUI=true
```
