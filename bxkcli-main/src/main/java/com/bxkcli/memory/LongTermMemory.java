...
public class LongTermMemory implements Memory {
    ...
    private static final String STORAGE_DIR_PROPERTY = "bxkcli.memory.dir";   //jvm参数
    private static final String STORAGE_DIR_ENV = "BXKCLI_MEMORY_DIR";        //环境变量
    private static final String STORAGE_FILE = "long_term_memory.json";
    private final Map<String, MemoryEntry> entries;
    // 线程安全的 int，在多线程环境下可以安全地自增、自减、更新，不需要加锁。
    ...
    public LongTermMemory(File storageDir) {
        ...
        this.mapper.enable(SerializationFeature.INDENT_OUTPUT);  //开启缩进输出
    ...
    public void store(MemoryEntry entry) {
        ...
        tokenCounter.addAndGet(entry.getTokenCount()); // 增加token数
        saveToDisk();   //每次存完都持久化
    }
    ...
    private void saveToDisk() {
        try {
            List<Map<String, Object>> dataList = entries.values().stream()
                    .map(this::entryToMap)
                    .collect(Collectors.toList());
            mapper.writeValue(storageFile, dataList); // 存储操作
        } catch (IOException e) {
    ...
    private static File resolveStorageDir() {
        ...
        return new File(new File(System.getProperty("user.home"), ".bxkcli"), "memory");
    }
