...
public abstract class AbstractOpenAiCompatibleClient implements LlmClient {
    ...
    protected static final OkHttpClient SHARED_HTTP_CLIENT = new OkHttpClient.Builder()
            .connectTimeout(readTimeoutSeconds("bxkcli.llm.connect.timeout.seconds", 60), TimeUnit.SECONDS)
            .readTimeout(readTimeoutSeconds("bxkcli.llm.read.timeout.seconds", 300), TimeUnit.SECONDS)
            .writeTimeout(readTimeoutSeconds("bxkcli.llm.write.timeout.seconds", 60), TimeUnit.SECONDS)
            .callTimeout(readTimeoutSeconds("bxkcli.llm.call.timeout.seconds", 600), TimeUnit.SECONDS)
            .build();
    ...
    public ChatResponse chat(List<Message> messages, List<Tool> tools, StreamListener listener) throws IOException {
        ...
        Request.Builder request = new Request.Builder()
                ...
                .post(body);
        customizeRequest(request);  // 子类可以追加自定义 header
        ...
        try (Response response = httpClient().newCall(builtRequest).execute()) {
            ...
            *   data: {"choices":[{"delta":{"content":"你好"}}]}
                ...
                data: [DONE]
            * */
            //数据从底层源（如网络、文件）读入内存缓冲区，再从中读取，从而减少系统调用次数，提升性能。
            BufferedSource source = responseBodyObj.source();
            String role = "assistant";
            StringBuilder content = new StringBuilder(); //累计正文
            StringBuilder reasoning = new StringBuilder(); //累计思考
            List<ToolCallAccumulator> toolAccumulators = new ArrayList<>(); // 累积工具调用
            int inputTokens = 0;    // 输入 token 数
            int outputTokens = 0;   // 输出 token 数
            int cachedInputTokens = 0; // 缓存命中的 token 数
            ...
            while (!source.exhausted()) {
                ...
                String trimmed = line.trim(); // 去除两边的空格
                if (trimmed.isEmpty() || !trimmed.startsWith("data:")) {
                    continue;
                }

                String payload = trimmed.substring("data:".length()).trim(); //去掉data前缀
                if (payload.isEmpty()) {
                    continue;
                }
                ...
                if (!reasoningDelta.isEmpty()) {
                    reasoning.append(reasoningDelta);   // 拼接思考内容
                    streamListener.onReasoningDelta(reasoningDelta);   // 实时回调给前端显示
                }

                String contentDelta = delta.path("content").asText("");
                if (!contentDelta.isEmpty()) {
                    content.append(contentDelta);   //拼接正文内容
    ...
    private ObjectNode buildRequestBody(List<Message> messages, List<Tool> tools) {
        ...
        requestBody.put("stream", true);  // 开启流式回复
    ...
    // 这个方法把 LLM 流式返回的 工具调用片段（delta） 累积拼接起来，因为 arguments 字段是分多次返回的。
