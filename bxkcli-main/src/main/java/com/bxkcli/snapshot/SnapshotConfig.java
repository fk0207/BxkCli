...
public record SnapshotConfig(
    ...
    public static SnapshotConfig fromEnvironment() {
        boolean enabled = readBoolean("bxkcli.snapshot.enabled", "BXKCLI_SNAPSHOT_ENABLED", true);
        Path root = Path.of(readString("bxkcli.snapshot.dir", "BXKCLI_SNAPSHOT_DIR",
                Path.of(System.getProperty("user.home"), ".bxkcli", "snapshots").toString()));
        int max = readInt("bxkcli.snapshot.max", "BXKCLI_SNAPSHOT_MAX", 50);
        List<String> excludes = mergeExcludes(readString("bxkcli.snapshot.excludes", "BXKCLI_SNAPSHOT_EXCLUDES", ""));
