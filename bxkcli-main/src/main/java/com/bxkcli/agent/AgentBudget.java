 ...
 * 1. 系统属性：{@code bxkcli.react.token.budget} / {@code bxkcli.react.stagnation.window} /
 *    {@code bxkcli.react.hard.max.iterations}
 ...
 * 场景（CI / 自动化批跑）通过 {@code -Dbxkcli.react.token.budget=N} 显式启用。
...
public class AgentBudget {
    ...
    private static final int DEFAULT_STAGNATION_WINDOW = 3; //停止检测
    private static final int DEFAULT_HARD_MAX_ITERATIONS = 50; //迭代数量超过上限
    ...
    public static AgentBudget fromLlmClient(LlmClient llmClient) {
        ...
        // 显式 -Dbxkcli.react.token.budget=N 仍可启用硬预算，覆盖默认。
        return new AgentBudget(
                readIntProperty("bxkcli.react.token.budget", Integer.MAX_VALUE),
                readIntProperty("bxkcli.react.stagnation.window", DEFAULT_STAGNATION_WINDOW),
                readIntProperty("bxkcli.react.hard.max.iterations", DEFAULT_HARD_MAX_ITERATIONS)
        );
    }
