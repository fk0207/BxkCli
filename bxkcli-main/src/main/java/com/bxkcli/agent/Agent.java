package com.bxkcli.agent;

import com.bxkcli.llm.LlmClient;
import com.bxkcli.llm.LlmTraceLogger;
import com.bxkcli.context.ContextProfile;
import com.bxkcli.context.TokenUsageFormatter;
import com.bxkcli.lsp.LspDiagnosticReport;
import com.bxkcli.memory.ConversationHistoryCompactor;
import com.bxkcli.memory.ExplicitMemoryHints;
import com.bxkcli.memory.MemoryManager;
import com.bxkcli.prompt.PromptAssembler;
import com.bxkcli.prompt.PromptContext;
import com.bxkcli.prompt.PromptMode;
import com.bxkcli.prompt.ProjectMemoryLoader;
import com.bxkcli.render.PlainRenderer;
import com.bxkcli.render.Renderer;
import com.bxkcli.render.StatusInfo;
import com.bxkcli.runtime.CancellationContext;
import com.bxkcli.skill.SkillContextBuffer;
import com.bxkcli.skill.SkillIndexFormatter;
import com.bxkcli.skill.SkillRegistry;
import com.bxkcli.util.AnsiStyle;
import com.bxkcli.tool.ToolRegistry;
import com.bxkcli.tool.ToolRegistry.ToolExecutionResult;
import com.bxkcli.tool.ToolRegistry.ToolInvocation;
import com.bxkcli.util.TerminalMarkdownRenderer;
import com.bxkcli.image.ImageReferenceParser;
...
public class Agent {
    ...
    public String run(String userInput) {
        ...
        pruneHistoricalImagePayloads(); // 遍历 conversationHistory，把历史消息中的图片内容（base64 编码）移除
        // 存入短期记忆
        memoryManager.addUserMessage(userInput); // 短期记忆是"会议纪要"，conversationHistory 是"录音"
        storeExplicitBrowserMemoryHint(userInput); //检查用户输入是否提到"浏览器"等关键词，如果有，存入记忆提示

        // 检索相关长期记忆，注入到 system prompt
        ContextProfile contextProfile = memoryManager.getContextProfile(); //拿到用户配置（比如记忆上下文 token 数、压缩阈值等
        String memoryContext = memoryManager.buildContextForQuery(userInput, contextProfile.memoryContextTokens()); //根据用户输入，从长期记忆（SQLite）检索相关内容
        updateSystemPromptWithMemory(memoryContext); // 把检索到的长期记忆注入到 conversationHistory 的第 0 条（system 消息）里

        // 添加用户输入到历史（如有 skill body 注入，前置到原文之前）
        String userMessageContent = prependSkillBodies(userInput); // 如果用户之前调过 /load_skill 加载技能，把技能内容前置到用户输入前面
        conversationHistory.add(ImageReferenceParser.userMessage(
                userMessageContent,
                Path.of(toolRegistry.getProjectPath())));
        StringBuilder reasoningTranscript = new StringBuilder(); // 累积整个 ReAct 循环中 LLM 的思考过程（reasoning content）
        StreamRenderer streamRenderer = new StreamRenderer(renderer());

        long startNanos = System.nanoTime(); // 记录 ReAct 循环开始时间，用于计算耗时
        AgentBudget budget = AgentBudget.fromLlmClient(llmClient); //  工厂方法，从系统属性读取配置创建预算对象
        pushStatus(budget, startNanos, "running"); // 底部状态栏显示当前状态

        // 主退出条件 = LLM 自己决定（不再调用工具就返回）；
        // budget 仅在 token 用尽 / 检测到死循环 / 超出硬轮数时兜底。
        while (true) {
            if (CancellationContext.isCancelled()) { //检查用户是否按了 Ctrl+C
                log.info("ReAct run cancelled before iteration");
                pushStatus(budget, startNanos, "idle"); // 底部状态栏显示当前状态
                return "⏹️ 已取消当前任务。"; // 返回取消提示
            }
            // 调 LLM 前评估 conversationHistory 是否接近 window 上限；超阈值就把早期消息压缩成摘要。
            // 真正决定下一轮 LLM input token 的是这里。
            injectPendingLspDiagnostics(); // 如果有 LSP（语言服务器协议）诊断信息（如代码语法错误），作为 user 消息加入历史
            maybeCompactHistory(); // 检查 conversationHistory 是否接近 token 上限
            ...
            if (exitReason != AgentBudget.ExitReason.WITHIN_BUDGET) {
                ...
                return "❌ " + description; // 返回错误信息给用户
            }

            int iteration = budget.beginIteration(); // 统计当前是第几轮迭代
            ...
            try {
                ...
                streamRenderer.beginThinking(); // 终端显示"Thinking..."状态，让用户知道正在思考
                // 调用 LLM
                ...
                LlmTraceLogger.logReasoning(log, "react iteration=" + iteration, llmClient, response.reasoningContent()); // 调试时可以看模型怎么思考的

                if (CancellationContext.isCancelled()) {
                    log.info("ReAct run cancelled after LLM response");
                    streamRenderer.finish(); // 结束流式渲染
                    pushStatus(budget, startNanos, "idle"); //状态栏设为空闲
                    return "⏹️ 已取消当前任务。";
                }
                ...
                if (response.hasToolCalls()) {
                    ...
                    budget.recordToolCalls(response.toolCalls()); // 记录本轮工具调用
                    // 添加助手消息（包含工具调用）
                    ...
                    streamRenderer.resetBetweenIterations(); // 清空 pending 缓冲区，重置渲染状态
                    renderer().appendToolCalls(response.toolCalls()); //终端显示即将执行的工具调用信息
                ...
                // 记录 token 使用
                memoryManager.recordTokenUsage(budget.totalInputTokens(), budget.totalOutputTokens(), budget.totalCachedInputTokens()); // memoryManager.recordTokenUsage()：把本次循环累计的 token 记入全局统计
                pushStatus(budget, startNanos, "idle"); // 告诉用户任务完成
    ...
    public String getContextStatus() {
        com.bxkcli.context.ContextProfile profile = memoryManager.getContextProfile();
        ...
        for (LlmClient.Message msg : conversationHistory) {
            int t = com.bxkcli.memory.TokenBudget.estimateMessagesTokens(java.util.List.of(msg));
            switch (msg.role()) {
    ...
    private int estimateToolsSchemaTokens() {
        try {
            return com.bxkcli.memory.MemoryEntry.estimateTokens(
    ...
    private long estimateCurrentContextTokens() {
        long messageTokens = com.bxkcli.memory.TokenBudget.estimateMessagesTokens(conversationHistory);
    ...
    private void logRequestContext(String scope, List<LlmClient.Tool> tools) {
        ...
        for (int messageIndex = 0; messageIndex < conversationHistory.size(); messageIndex++) {
            ...
            messages++;
            int tokens = com.bxkcli.memory.TokenBudget.estimateMessagesTokens(List.of(msg));
        ...
        if (tools != null && !tools.isEmpty()) {
            try {
                toolsSchemaTokens = com.bxkcli.memory.MemoryEntry.estimateTokens(
