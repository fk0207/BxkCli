...
public final class WechatPaths {
    ...
    public static Path root() {
        String configured = System.getProperty("bxkcli.wechat.dir");
        if (configured == null || configured.isBlank()) {
            configured = System.getenv("BXKCLI_WECHAT_DIR");
        }
        if (configured == null || configured.isBlank()) {
            configured = Path.of(System.getProperty("user.home"), ".bxkcli", "wechat").toString();
        }
    ...
    public static Path pidFile() {
        return root().resolve("bxkcli-wechat.pid");
    }
}
