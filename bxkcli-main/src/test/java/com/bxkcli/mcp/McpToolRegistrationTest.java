...
class McpToolRegistrationTest {
    ...
    private static void withAuditDir(Path tempDir, ThrowingRunnable body) throws Exception {
        String previous = System.getProperty("bxkcli.audit.dir");
        System.setProperty("bxkcli.audit.dir", tempDir.resolve("audit").toString());
        try {
            body.run();
        } finally {
            if (previous == null) {
                System.clearProperty("bxkcli.audit.dir");
            } else {
                System.setProperty("bxkcli.audit.dir", previous);
            }
        }
    }
